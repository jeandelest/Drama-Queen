...
import {PullData} from "../../../hooks/usePullData";
 ...
 */
export const storeSyncProgress = (data: PullData | null = null, errors: SyncError[] = []) => {
    if (data === null) {
        window.localStorage.setItem(LOCALSTORAGE_KEY, JSON.stringify({ error: 'pending' } satisfies Store));
        return;
    }
